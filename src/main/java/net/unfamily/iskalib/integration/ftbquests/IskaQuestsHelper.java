...
final class IskaQuestsHelper {
    ...
    static void addCurrencySelector(ConfigGroup config, String current, java.util.function.Consumer<String> setter, String nameKey) {
        List<String> ids = ShopCurrencyHooks.getListener().listCurrencyIds();
        if (!ids.isEmpty()) {
            String selected = ids.contains(current) ? current : ids.get(0);
            config.addEnum(
                    "currency",
                    selected,
                    setter,
                    NameMap.of(selected, ids.toArray(new String[0]))
    ...
    static void addScopeSelector(ConfigGroup config, String current, java.util.function.Consumer<String> setter, String nameKey) {
